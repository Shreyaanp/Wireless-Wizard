    ...
    while 1:                                            # While loop to continuesly scan and read data from serial port and execute
        ...
        if data[0] == "DATAB":                          # Checking if the identifier is "DATAB" which the Arduino sends the values for Left/Right button
              if data[1] == 'L' :                       # If the Left button is pressed
                mouse.press(but.left)                # The corresponding but is pressed and released
                mouse.release(but.left)
              if data[1] == 'R' :                       # If the Right but is pressed
                      mouse.press(but.right)         # The corresponding but is pressed and released
...
try:
  ...
  while 1:                                        # While loop to continuesly scan and read data from serial port and execute
      ...
      if(len(data)==2):
        if data[0] == "DATAB":
                                  # Checking if the identifier is "DATAB" which the Arduino sends the values for Left/Right but
              if data[1] == 'L' :                       # If the Left but is pressed
                mouse.press(but.left)                # The corresponding but is pressed and released
                mouse.release(but.left)
              if data[1] == 'R' :                       # If the Right but is pressed
                      mouse.press(but.right)         # The corresponding but is pressed and released
